...
import { ComponentShort, statusComponent, getTimeAgo } from '@/services/organizations'
...
onMounted(() => {
    statusComponent(props.organizationId, props.applicationId, props.environmentId, props.component.name)
        .then((response) => {
...
<template>
    <div class="w-80 h-40 border border-color rounded-md p-3 cursor-pointer">
        ...
        <div class="mt-3 pb-2 border-b border-color">
            ...
            <div class="w-40 h-3 mt-2 rounded-sm skeleton" :class="{ hidden: !isLoading }"></div>
            <div class="font-bold text-xs text-gray-500" :class="{ hidden: isLoading}">Last update {{ getTimeAgo(date) }} ago</div>
        </div>
