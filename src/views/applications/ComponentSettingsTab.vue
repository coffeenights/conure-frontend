...
const schemaSettings = {
  ...
  sourceImage: z.string().max(255),
  sourceCommand: z.string().max(255).optional(),
...
<template>
  <form @submit.prevent="onSubmit">
        ...
        >
          <div class="grow w-full space-y-2">
            ...
            <div class="flex justify-between text-sm text-muted-foreground">
              <span>{{
                component.settings.resources_settings.replicas[0]
              }}</span>
            </div>
            <SettingsErrorMessage />
          </div>
    ...
    <div class="flex w-full my-5">
      <Button type="submit" :disabled="isSubmitting" class="grow"
        >Save</Button
      >
    </div>
  </form>
